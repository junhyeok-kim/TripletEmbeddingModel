...
class TripletEmbedding(object):
    ...
    def ContrastiveMaxMarginLoss(self, x, y,
                                 T1, T2, T3, W1, W2, W3, mu):
        """Build the graph for the Margin Loss"""
        margin_loss = tf.reduce_sum(
                tf.maximum(tf.constant(0., dtype=tf.float32),
                           tf.subtract(tf.constant(self._margin, dtype=tf.float32),
                                       tf.subtract(x, y))))
    ...
    def generate_batch_w_corrupted(self, vecS, vecV, vecO,
        ...
        return batch_s, batch_v, batch_o, batch_c
    ...
    def build_graph(self):
        ...
        c_input    = tf.placeholder(tf.float32, shape=(None, d))
        c_v_input  = tf.concat([c_input, v_input], 1)
...
#        init_width = 0.5 / d
        ...
        _, R1_c = self.NTNLayer(c_input, v_input, c_v_input, T1, W1, b1)
        R1R2_c = tf.concat([R1_c, R2], 1)
        _, _, s_corrupted_score = self.EELM(R1_c, R2, R1R2_c, T3, W3, b3, mu)
        self.s_corrupted_score = s_corrupted_score

        # loss function
        margin_loss, loss = self.ContrastiveMaxMarginLoss(event_score,
                                                     s_corrupted_score,
                                                     T1, T2, T3, W1, W2, W3, mu)
        # optimizer
    ...
    def train(self, vecS, vecV, vecO):
        ...
        for N in range(self._epoch):
            ...
            for i in range(iter_num):
                ...
                idx_used = np.random.choice(idx_set, size=batch_size,
                                            replace=False)
                batch_s, batch_v, batch_o, batch_c = self.generate_batch_w_corrupted(
                                         vecS, vecV, vecO,
                                         batch_size=batch_size, C=self._C,
                                         idx_used=idx_used)
        
                feed_batch = {self.s_input: batch_s,
                              self.v_input: batch_v,
                              self.o_input: batch_o,
                              self.c_input: batch_c,
                              self.lr: lr}
        ...
        # ---------------------------------------------------------------------
            ...
            logger.info(str(N+1) + " Epoch loss: " + str(np.sum(epoch_loss)))
            logger.info(str(N+1) + " Epoch Margin loss: " + str(np.sum(epoch_margin_loss)))
